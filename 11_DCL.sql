 ...
 * 		EXECUTE			 		   PROCEDURE (실행권한 - 프로시저(함수)를 실행할 권한)
 * 
 * */
...
--[작성법]
--CREATE USER 사용자명 IDENTIFYIED BY 비밀번호;

CREATE USER LHY_SAMPLE IDENTIFIED BY 1234;
...
GRANT CREATE SESSION TO LHY_SAMPLE;
...
--6.다시 SYS테이블로 돌아가서 할당권한 부여 
--(SYS)테이블 생성 권한 부여 + TABLESPACE 할당
GRANT CREATE TABLE TO LHY_SAMPLE;

ALTER USER LHY_SAMPLE DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;

--7. (SAMPLE로 돌아와서) 테이블 다시 생성
CREATE TABLE TB_TEST(
PK_COL NUMBER PRIMARY KEY,
CONTENT VARCHAR2(100)
);
...
--CONNECT / RESOURCE : 사용자에게 부여할 수 있는 기보적인 시스템 역할.
...
GRANT CONNECT , RESOURCE TO LHY_SAMPLE;
...
--2. (KH) SAMLE 계정에 EMPLOYEE 테이블 조회 권한 부여
--[객체 권한 부여 방법]
--GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO LHY_SAMPLE;

-- 3. (SAMPLE) 다시 KH 계정이 가진 EMPLOYEE 조회
SELECT * FROM KH.EMPLOYEE; --> 조회 잘 된다!
SELECT * FROM KH.DEPARTMENT; --> 조회 안됨
-->위에서 KH 가 EMPLOYEE 조회하는 권한만 주었기 때문에 DEPARTMENT는 안됨
...
REVOKE SELECT ON EMPLOYEE FROM LHY_SAMPLE;

--5.(SAMPLE) EMPLOYEE 조회해보기
SELECT * FROM KH.EMPLOYEE;-->권한을 회수해서 조회안됨.
