...
--DB에 반영된 것 처럼 보이지만
--실제로 아직 DB에 영구반영 된것이 아니라 트렌젝션에 남아있음
--트렌잭션에 INSERT 3개가 들어가져 있음

--ROLLBACK 후 확인
ROLLBACK; --마지막 커밋시점까지 되돌아감.

SELECT * FROM DEPARTMENT2;
--개발1팀,개발2팀,개발3팀 전부 롤백됨
...
--ROLLBACK TO "SP2"; 
--ROLLBACK TO "SP1" 구문 수행시 이후에 설정된 SP2,SP3도 전부 날아가서 실행안됨
...
SELECT * FROM DEPARTMENT2;
--개발 6팀만 없음 (SP2 지점 SAVEPOINT 전 개발 5팀 개발 4팀 있는상태에서 롤백해서)
