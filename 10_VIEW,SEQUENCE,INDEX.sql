...
/* VIEW를 이용해서 DML 사용하기 + 문제점 확인 */ --사용은 가능하나 사용하지 않는다
...
START WITH 100 --시반번호 100
INCREMENT BY 5 -- NEXTVAL 이 호출되면 5씩증가 만약 감소하고싶으면 -5
...
-->CURRVAL 정확한 의미는 가장 최근 호출된 NEXTVAL의 값을 반환함을 뜻함
...
--SEQUENCE 변경 (ALTER)
--> START WITH 빼고 모두 변경 가능! 시작값은 변경 불가능
...
FROM EMPLOYEE;
-- ROWID : 오라클에서 각 행에 고유한 주소를 나타내는 가상 컬럼(물리적 주소)
--인덱스가 ROWID 저장함.
...
SELECT EMP_NAME 
FROM EMPLOYEE
WHERE ROWID = 'AAAR9jAABAAAZdxAAB' ;
