...
--DEPARTMENT 테이블 복사 (컬러명 , 데이터타입 , NOTNULL 조건만 복사)
...
--컬럼자체에 NULL 허용 / 비허용 제어하는 성질 변경의 형태로 인식됨.

--MODIFY (수정하다) 구문을 사용해서 NULL 제어
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE NOT NULL; -- DEPT TITLE 컬럼의 NOT NULL 제약조건 적용;


ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE NULL; -- DEPT TITLE 컬럼의 NULL 제약조건 적용
...
--DEPT_ID 컬럼 데이터 탑입 수정
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(3);

INSERT INTO DEPT_COPY 
VALUES ('D10','개발1팀','L1',NULL,DEFAULT);
--컴럼 데이터 타입 수정후 다시 위 INSERT 실행 -> 삽입성공
...
--DEPARTMENT 테이블 복사해서 DEPT_COPY 테이블 생성
CREATE TABLE DEPT_COPY
AS SELECT * FROM DEPARTMENT;
--컬럼명,데이터타입,NOTNULL여부만 복사
...
/* DDL 주의 사항 */
-- 1) DDL 은 COMMIT 과 ROLLBACK 이 되지 않는다.
-- 2)DDL과 DML 구문 섞어서 수행하면 안된다!
-- DDL(CREATE, ALTER , DROP : 객체 생성/수정/삭제)
-- DML(INSERT.UPDATE,DELET : 데이터(행) 추가/갱신/삭제)
...
SELECT * FROM TB2;
--롤백이 안된다 ..위에서 DDL 구문주 ALTER를 사용해서 그시점에 강제 COMMIT이 되어버림