...
[JOIN 용어 정리]
  오라클                                   SQL : 1999표준(ANSI)
----------------------------------------------------------------------------------------------------------------
등가 조인                               내부 조인(INNER JOIN), JOIN USI NG / ON
...
--> 연결되는 컴럼의 값이 일치하는 행들만 조인됨.
--> 일치하는 값이 없는 행은 조인에서 제외됨.

-- 작성 방법은 크게 ANSI 구문과 오라클 구문으로 나뉘고
--ANSI USING 과 ON을 쓰는 방법으로 나뉜다.
...
FROM DEPARTMENT d
JOIN LOCATION ON (d.LOCATION_ID = LOCATION.LOCAL_CODE);
...
FROM EMPLOYEE LEFT /*OUTER (생갹 가능)*/ JOIN DEPARTMENT d 
ON(DEPT_CODE = d.DEPT_ID ); --23행(DEPT_CODE가 NULL인 하동운,이오리 포함)

--오라클
SELECT EMP_NAME,d.DEPT_TITLE 
FROM EMPLOYEE e ,DEPARTMENT d 
WHERE DEPT_CODE(+) = d.DEPT_ID ; --LEFT 정렬



--ANSI
SELECT EMP_NAME,DEPT_TITLE
FROM EMPLOYEE RIGHT /*OUTER*/ JOIN DEPARTMENT d 
ON(DEPT_CODE = d.DEPT_ID );

--오라클
SELECT EMP_NAME,d.DEPT_TITLE 
FROM EMPLOYEE e ,DEPARTMENT d 
WHERE DEPT_CODE= d.DEPT_ID(+) ; --RIGHT 정렬
...
--개발자가 의도해서 작성할일은 거의없고 조인구문에서 잘못작성해서 크로스 조인이 일어남
...
--테이블마다 별칭 작성(미작성시 열의 정의가 애매하다는 오류가 발생)
...
WHERE DEPT_CODE = d.DEPT_ID --(EMPLOYEE + DEPARTMENT조인)
AND d.LOCATION_ID = l.LOCAL_CODE ;--(EMPLOYEE + DEPARTMENT조인)+LOCATION 조인

--[다중 조인 연습 문제]

-- 직급이 대리이면서 아시아 지역에 근무하는 직원 조회(ASIA 시작하는 지역명)
--사번,이름,직급명,부서명,근무지역명,급여
-- EMPLOYEE EMP_ID , EMP_NAME , JOB_CODE ,  LOCATION.LOCAL_CODE ,  SARALY
																--JOB       DEPARTMENT.LOCATION_ID
--EMPLOYEE
--JOB
--DEPARTMENT
--LOCATION
...
JOIN DEPARTMENT d  ON(EMPLOYEE.DEPT_CODE = d.DEPT_ID )
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
WHERE JOB_NAME = '대리'
AND LOCATION.LOCAL_NAME LIKE 'ASIA%';

--JOIN 연습문제
--1. 주민번호가 70년대 생이면서 성별이 여자이고, 성이 '전'씨인 직원들의
--사원명, 주민번호, 부서명, 직급명을 조회하시오.
SELECT E.EMP_NAME,E.EMP_NO,D.DEPT_CODE,J.JOB_CODE
FROM EMPLOYEE E
