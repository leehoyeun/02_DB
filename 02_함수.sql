...
--문자열을 5번째 문자부터 검색하여 두번째 B의 위치검색
...
-- EMPLOYEE 테이블 에서 사원의 월급을 100만으로 나눴을때 나머지를 조회
...
SELECT ROUND(123.456,1)FROM DUAL;---123,소수점 첫번째자리에서 반올림
...
SELECT TRUNC (123.456,1) FROM DUAL; --123.4,소수점 첫째자리 절삭
...
SELECT SYSDATE FROM DUAL; --2025-03-10 12:10:56.000
...
SELECT ABS(ROUND(MONTHS_BETWEEN(SYSDATE,'2025-07-22'),3))"수강 기간(계월)"
FROM DUAL;
...
SELECT ADD_MONTHS(SYSDATE,4)FROM DAUL;
...
--지정된 포맷으로 날짜를 인식함
SELECT TO_DATE('2025-03-10')FROM DUAL; --2025-03-10 00:00:00.000 문자로 전달
SELECT TO_DATE(20250310) FROM DUAL; --숫자로전달

SELECT TO_DATE('200310 140730','YYMMDD HH24 MI SS') FROM DUAL;
--SQL Error [1861] [22008]: ORA-01861: 리터럴이 형식 문자열과 일치하지 않음
--> 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날짜 형식인지 인식 시킴

--Y 패턴 : 현재 세기(21세기 == 20XX년대 ==2000년대)
--R 패턴 : 1세기 기준으로 절반(50년) 이상인 경우 이전세기(1900년)
...
----EMPLOYEE 테이블에서 각 지원이 태어난 생년월일 조회
...
--숫자 형변환
--TO_NUMBER(문자형데이터,[포맷]) : 문자형 데이터를 숫자 데이터로 변경
...
-- 서브쿼리 +그룹함수
SELECT MAX(SALARY)FROM EMPEMPLOYEE;

--COUNT():행 개수를 헤아려서 반환
--COUNT(컬럼명) : NULL을 제외한 실제값이 기록된 행 개수를 리턴
--COUNT(*) : NULL을 포함한 전체 행 개수를 리턴
--COUNT([DISTINCT]컬러명):중복을 제거한 행 개수를 리턴
