...
--1) 부서코드만 조회
SELECT DEPT_CODE FROM EMPLOYEE ;  --23개행
...
FROM EMPLOYEE
GROUP BY DEPT_CODE;
--DEPT_CODE컬럼을 그룹으로 묶어, 그 그룹의 급여 합계(SUM(SALARY))를 구함

--EMPLOYEE 테이블에서(FROM)
--직급코드가 같은 사람의(JOB_CODE)
--직급코드 , 급여평균,인원 수를 (SELECT 절)
--집급코드 오름차순으로 조회 (ORDER BY 절)
...
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5','D6') --부서코드가 D5,D6거름
GROUP BY DEPT_CODE; --WHERE 절에서 구한 부서코드가 D5 , D6인 데이터들을 그룹으로 묶음


--EMPLOYEE 테이블에서 (FROM절)
--2000년도 이후 입사자들의(WHERE절 -> GROUP BY)  
--직급별 급여 합을 조회 (SELECT절)
SELECT JOB_CODE"직업",SUM(EMPLOYEE.SALARY)"급여"
FROM EMPLOYEE
...
--그룹 함수가 적용되지 않은 컬럼을
-- 모두 GROUP BY 절에 작성되어 있어야만 한다


--EMPLOYEE 테이블에서   (FROM절)
--부서별로 같은 직급인 사원의 인원수를 조회 (GROUP BY 절 DEPT_CODE,JOB_CODE묶음) 그룹내 그룹
...
--EMPLOYEE 테이블에서 (FROM)
--부서별 평균 급여가 3백만원 이상인 부서의 GROUP BY 절 
--부서코드,평균조회 SELECT
--부서코드 오름차순 ORDER BY
...
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 --> DEPT 그룹 중 급여 평균이 3백만원 이상인 그룹만 조회
ORDER BY DEPT_CODE ;
...
--CUBE : GROUP BY 절에 모든 컴럼의 중간 집계를 처리하는 함수
SELECT DEPT_CODE,JOB_CODE,COUNT(*)
FROM EMPLOYEE
...
/* 집합 연산자 (SET OPERATOR)
 * 
 * --여러 SELECT의 결과 (RESULT SET)을 하나의 결과로 만드는 연산자
 * 
...
--UNION ALL
--MINUS
-- (2번째 SELECT문)급여가 300만원 초과인 사원의 사번,이름,부서코드,급여조회
SELECT EMP_ID,EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE 
WHERE SALARY > 3000000;

-- (주의사항!) 집합 연산자를 사용하기 위한 SELECT문들은
-- 조회하는 컬럼의 타임,개수가 모두 동일해야 한다!
